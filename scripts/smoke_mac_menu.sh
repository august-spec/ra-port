...
Build and launch the macOS Red Alert port from the repo root long enough to capture the title/menu.
...
for window in windows {
    ...
    if owner == "redalert_mac" || name.contains("command & conquer: red alert") {
...
pkill -TERM -x redalert_mac 2>/dev/null || true
pkill -9 -x UserNotificationCenter 2>/dev/null || true
pkill -9 -x CoreServicesUIAgent 2>/dev/null || true

cd "$ROOT_DIR"
"$BUILD_DIR/redalert_mac" > "$LOG_FILE" 2>&1 &
pid=$!

sleep "$SECONDS_TO_WAIT"
window_id="$(redalert_window_id | head -n 1)"
if [[ -n "$window_id" ]]; then
...
if kill -0 "$pid" 2>/dev/null; then
  echo "redalert_mac still running after ${SECONDS_TO_WAIT}s"
  ...
  wait "$pid"
  status=$?
  echo "redalert_mac exited before screenshot with status $status" >&2
  exit "$status"
fi

...
static void test_tap_clicks_on_release(void)
{
	...
	AndroidTouchGesture_Begin(&touch, 1, 100, 120, 1000, &out);
	assert(out.count == 0);

	AndroidTouchGesture_End(&touch, 1, 100, 120, &out);
	assert(out.count == 2);
...
static void test_drag_holds_left_button_after_slop(void)
{
	...
	AndroidTouchGesture_Init(&touch);
	AndroidTouchGesture_Begin(&touch, 4, 50, 60, 2000, &out);
	AndroidTouchGesture_Move(&touch, 4, 70, 60, &out);
	assert(out.count == 2);
	assert(out.events[0].type == ANDROID_TOUCH_LEFT_DOWN);
	assert(out.events[0].x == 50);
	assert(out.events[0].y == 60);
	assert(out.events[0].update_cursor == 1);
	assert(out.events[1].type == ANDROID_TOUCH_MOUSE_MOVE);
	assert(out.events[1].x == 70);
	assert(out.events[1].y == 60);

	AndroidTouchGesture_End(&touch, 4, 75, 65, &out);
	assert(out.count == 2);
...
static void test_long_press_is_right_click(void)
{
	...
	AndroidTouchGesture_Init(&touch);
	AndroidTouchGesture_Begin(&touch, 7, 30, 40, 3000, &out);
	AndroidTouchGesture_Update(&touch, 3650, &out);
	assert(out.count == 2);
	assert(out.events[0].type == ANDROID_TOUCH_RIGHT_DOWN);
	assert(out.events[1].type == ANDROID_TOUCH_RIGHT_UP);
	assert(out.events[0].update_cursor == 0);
	assert(out.events[1].update_cursor == 0);

	AndroidTouchGesture_End(&touch, 7, 30, 40, &out);
	assert(out.count == 0);
}
...
static void test_small_motion_does_not_hover(void)
{
	...
	AndroidTouchGesture_Init(&touch);
	AndroidTouchGesture_Begin(&touch, 8, 10, 20, 4000, &out);
	AndroidTouchGesture_Move(&touch, 8, 13, 23, &out);
	assert(out.count == 0);

	AndroidTouchGesture_End(&touch, 8, 13, 23, &out);
	assert(out.count == 2);
	assert(out.events[0].type == ANDROID_TOUCH_LEFT_DOWN);
	assert(out.events[0].update_cursor == 0);
	assert(out.events[1].type == ANDROID_TOUCH_LEFT_UP);
...
static void test_two_finger_pan_cancels_tap(void)
{
	...
	AndroidTouchGesture_Init(&touch);
	AndroidTouchGesture_Begin(&touch, 9, 40, 50, 5000, &out);
	AndroidTouchGesture_Begin(&touch, 10, 80, 90, 5010, &out);
	assert(out.count == 0);

	AndroidTouchGesture_End(&touch, 9, 42, 52, &out);
	assert(out.count == 0);

	AndroidTouchGesture_Update(&touch, 5700, &out);
	assert(out.count == 0);

	AndroidTouchGesture_End(&touch, 10, 82, 92, &out);
	assert(out.count == 0);
}
